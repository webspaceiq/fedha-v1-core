...
describe("Governance Execute", function () {
    ...
    it("Should allow deployer to queue a proposal", async function () {
        const [deployer, notDeployer, voter1, voter2] = await ethers.getSigners();
    ...
    it("Should allow anyone to queue a proposal", async function () {
    ...
    it("Should return proposal state as Queued", async function () {
        ...
        await governorInstance.connect(notDeployer).queue(
            [tokenInstance.address],
            [0],
            [proposalData.transferCalldata],
            ethers.utils.id(proposalData.descriptionHash),
        ...
        expect(state).to.equal(GovernanceProposalState.Queued);
    });
});
